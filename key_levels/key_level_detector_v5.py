...
class KeyLevelDetectorV5:
    """
    V5 Key Level Detector - VISIBLE STRONG REVERSALS ONLY

    KEY REQUIREMENTS:
    - Price must touch zone from a clear direction (from above or below)
    - Price must REVERSE STRONGLY (move at least 2-3% away from zone)
    - Minimum 3 touches with 75%+ reversal rate
    - Reversals must be VISIBLE on monthly chart
    """
    ...
    def find_key_levels(self, df, symbol, num_levels=4):
        ...
        for zone_height_pct in zone_heights:
            ...
            while current < price_max:
                ...
                # V5: Test this zone with STRONG reversal detection
                result = self.test_zone_v5(df, zone_bottom, zone_top)

                # V5: Minimum 3 touches, 75% reversal rate (visible reversals only)
                if result['touches'] >= 3 and result['reversal_rate'] >= 75:
                    # Score
        ...
        print(f"Found {len(all_zones)} candidate zones (3+ touches, 75%+ reversal, VISIBLE reversals)")
    ...
    def test_zone_v5(self, df, zone_bottom, zone_top):
        """V5: STRONG VISIBLE REVERSALS - price must move significantly away from zone"""
        zone_mid = (zone_top + zone_bottom) / 2
        ...
        # V5: Minimum reversal distance (2.5% move away from zone)
        ...
        while i < len(df) - 5:  # Need more bars ahead to verify reversal
            ...
            # V5: Did price touch this zone?
            if bar_low <= zone_top and bar_high >= zone_bottom:

                # V5: Check WHERE price came from (look back 2-3 bars)
                ...
                for lookback in range(1, 4):
                    if i >= lookback:
                        ...
                        if prev_close > zone_top * 1.01:  # Clearly above zone
                            came_from_above = True
                            break
                        elif prev_close < zone_bottom * 0.99:  # Clearly below zone
                ...
                # V5: Check for STRONG reversal (price must move 2.5%+ away from zone)
                if i < len(df) - 5:
                    if came_from_above:
                ...
                # V5: Skip next 3 bars to avoid counting same touch multiple times
                i += 4
            else:
                i += 1
    ...
    def print_levels(self, zones, symbol):
        ...
        print(f"\n{'='*70}")
        print(f"{symbol} - DETECTED {len(zones)} KEY LEVELS (V5 - VISIBLE REVERSALS)")
        print(f"{'='*70}\n")
    ...
    def visualize(self, df, zones, symbol):
        ...
        ax.set_title(f'{symbol} - Key Levels V5 (Visible Reversals)', fontsize=15, fontweight='bold')
        ax.grid(True, alpha=0.3)

        plt.tight_layout()
        filename = f'{symbol}_key_levels_v5.png'
        plt.savefig(filename, dpi=150)
        print(f"Chart saved: {filename}")
        plt.close()

if __name__ == "__main__":
    detector = KeyLevelDetectorV5()
