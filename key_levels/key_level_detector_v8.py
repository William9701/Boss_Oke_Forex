...
class KeyLevelDetectorV8:
    """
    V8 INSTITUTIONAL MONTHLY KEY LEVEL DETECTOR

    STRICT INSTITUTIONAL REQUIREMENTS (20+ years experience):
    ============================================================
    1. 10-20 YEAR lookback minimum (120-240+ monthly candles)
    2. MAJOR swing pivots only (5-bar strength minimum)
    3. MINIMUM 3-4 confirmed reactions with REAL reversals (500-1500+ pips)
    4. Zone clustering: 3+ swing points within 0.4-1.2x monthly ATR(14)
    5. Confluence required: Round numbers, major cycle highs/lows, Fib levels
    6. No recent decisive breaks (>1.5x ATR in last 12-24 months)
    7. Battle-tested over YEARS - reject anything without multi-year history
    8. Output only TOP 4-7 strongest zones - NO CLUTTER

    SCORING (1-10): Touches 40% | Reversal Quality 30% | Confluence 20% | Recency 10%
    """
    ...
    def calculate_atr(self, df, period=14):
        ...
        return atr.iloc[-1] if len(atr) > 0 else 0.01

    def is_round_number(self, price, tolerance=0.002):
        """Check if price is near psychological round number"""
        # Check for major round numbers: 1.0000, 1.5000, 2.0000, 100.00, 150.00
        round_levels = []

        # For pairs like EURUSD (around 1.0-2.0)
        for i in range(0, 30):
            round_levels.append(0.5000 + i * 0.1000)  # 0.5, 0.6, 0.7...
            round_levels.append(0.5000 + i * 0.0500)  # 0.5, 0.55, 0.6...

        # For pairs like USDJPY (around 80-160)
        for i in range(50, 200):
            round_levels.append(float(i))  # 50, 51, 52...
            if i % 5 == 0:
                round_levels.append(float(i))  # Extra weight for 5s

        # Check if price is within tolerance of any round level
