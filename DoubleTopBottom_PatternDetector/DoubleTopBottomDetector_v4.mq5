//+------------------------------------------------------------------+
//|                                   DoubleTopBottomDetector_v4.mq5 |
//|                         Professional Double Top/Bottom Detection |
//|                                   Monthly Data - All Timeframes  |
//|                                                                  |
//| V4 Changes:                                                      |
//| - Fixed intersection check with tolerance buffer                 |
//| - Line can touch price but not significantly cross through it    |
//| - Allows valid double bottoms that were previously rejected      |
//+------------------------------------------------------------------+
...
//--- Input parameters - OPTIMIZED VALUES V4
...
input double IntersectionTolerance = 1.5;  // V4: Tolerance % for line intersection
...
int OnInit()
{
   Print("=== Double Top/Bottom Detector V4 Started ===");
   ...
   Print("V4: Line intersection check with ", IntersectionTolerance, "% tolerance");
...
//+------------------------------------------------------------------+
// V4: IMPROVED - Check if line SIGNIFICANTLY intersects chart (with tolerance)
bool LineIntersectsChart(int firstIdx, int secondIdx, double linePrice, bool isTop)
{
   // Calculate tolerance buffer
   ...
   for(int k = firstIdx + 1; k < secondIdx; k++)
   {
      if(isTop)
      {
         // For double top, check if any HIGH goes SIGNIFICANTLY above the line
         // Allow small touches within tolerance
         if(g_monthlyHigh[k] > (linePrice + toleranceBuffer))
         {
            return true; // Line cuts through significantly - invalid pattern
         }
      }
      else
      {
         // For double bottom, check if any LOW goes SIGNIFICANTLY below the line
         // Allow small touches within tolerance
   ...
   return false; // Line doesn't cut through significantly - valid pattern
}
...
int DetectDoubleTops()
{
   ...
   for(int i = 0; i < g_numSwingHighs; i++)
   {
      for(int j = i + 1; j < g_numSwingHighs; j++)
      {
         ...
         // V4: Check if line SIGNIFICANTLY intersects chart (with tolerance)
         double linePrice = avgHigh;
