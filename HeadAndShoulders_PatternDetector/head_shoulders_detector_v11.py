...
# Parameters - V11 WITH HEAD PROMINENCE PRIORITY
SYMBOL = "AUDUSD"
LOOKBACK_BARS = 240  # 20 years
SWING_ORDER = 5

# V11: Professional criteria
...
print(f"=== V11 Head & Shoulders Detection on {SYMBOL} (HEAD PROMINENCE PRIORITY) ===\n")
print(f"V11: Prioritize patterns with HIGHEST head prominence and price\n")
...
print("[V11 DEBUG] Testing L-H-R combinations for Bearish H&S:")
...
for i, (ls_idx, ls_high, ls_prom) in enumerate(prominent_highs[:-2]):
    for j, (h_idx, h_high, h_prom) in enumerate(prominent_highs[i+1:-1], start=i+1):
        for k, (rs_idx, rs_high, rs_prom) in enumerate(prominent_highs[j+1:], start=j+1):
            ...
            # V11: MODIFIED SCORING - Prioritize head prominence and absolute price
            # Add massive bonus for having the highest head price among all candidates
            score = (head_height * 5.0) + (h_prom * 10.0)  # V11: Increased prom weight from 3.0 to 10.0
            score += (h_high * 100.0)  # V11: NEW - Add absolute head price as bonus
            score -= shoulder_diff * 0.3
            score -= neckline_diff * 5.0
            if has_rsi_div:
                score += 50.0
            ...
            # V11: DEBUG - Print this candidate
            ...
            print(f"    Score={score:.2f}, shoulder_diff={shoulder_diff:.1f}%, head_height={head_height:.1f}%, h_prom={h_prom:.1f}%")
...
ax1.set_title(f'{SYMBOL} - V11 H&S Detection (HEAD PROMINENCE PRIORITY)\nPrioritize Patterns with Highest Head Price and Prominence',
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v11_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V11] Chart saved to: {output_path}")
