...
# Parameters - V15 WITH SNAP-TO-ADJACENT SHOULDERS
SYMBOL = "AUDUSD"
...
print(f"=== V15 Head & Shoulders Detection on {SYMBOL} (SNAP-TO-ADJACENT) ===\n")
print(f"Including: ZigZag + Proper Head & Shoulder Detection\n")
print(f"V15: Detect pattern first, then snap shoulders to adjacent swings\n")
...
print("=" * 80)
print("STEP 5A: INVERSE H&S PATTERN DETECTION (V15)")
print("=" * 80)
...
if best_inverse_pattern:
    ...
    if head_idx_in_list is not None and head_idx_in_list > 0 and head_idx_in_list < len(prominent_lows) - 1:
        ...
        print(f"[V15] SNAPPING shoulders to adjacent swings:")
        ...
        # Recalculate neckline with new shoulders
        peak1_idx = new_ls_idx
        peak1_high = highs_denoised[new_ls_idx]
        for idx in range(new_ls_idx, best_inverse_pattern['h_idx'] + 1):
            if highs_denoised[idx] > peak1_high:
                peak1_high = highs_denoised[idx]
                peak1_idx = idx

        peak2_idx = best_inverse_pattern['h_idx']
        peak2_high = highs_denoised[best_inverse_pattern['h_idx']]
        for idx in range(best_inverse_pattern['h_idx'], new_rs_idx + 1):
            if highs_denoised[idx] > peak2_high:
                peak2_high = highs_denoised[idx]
                peak2_idx = idx
...
print("=" * 80)
print("STEP 5B: BEARISH H&S PATTERN DETECTION (V15)")
print("=" * 80)
...
if best_bearish_pattern:
    ...
    if head_idx_in_list is not None and head_idx_in_list > 0 and head_idx_in_list < len(prominent_highs) - 1:
        ...
        # Recalculate neckline with new shoulders
        valley1_idx = new_ls_idx
        valley1_low = lows_denoised[new_ls_idx]
        for idx in range(new_ls_idx, best_bearish_pattern['h_idx'] + 1):
            if lows_denoised[idx] < valley1_low:
                valley1_low = lows_denoised[idx]
                valley1_idx = idx

        valley2_idx = best_bearish_pattern['h_idx']
        valley2_low = lows_denoised[best_bearish_pattern['h_idx']]
        for idx in range(best_bearish_pattern['h_idx'], new_rs_idx + 1):
            if lows_denoised[idx] < valley2_low:
                valley2_low = lows_denoised[idx]
                valley2_idx = idx
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v15_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V15] Chart saved to: {output_path}")
