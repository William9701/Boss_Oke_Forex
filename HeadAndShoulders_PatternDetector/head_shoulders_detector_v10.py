...
# Parameters - V10 WITH REFINED HEAD VALIDATION
SYMBOL = "AUDUSD"
LOOKBACK_BARS = 240  # 20 years
SWING_ORDER = 5

# V10: Professional criteria
...
print(f"=== V10 Head & Shoulders Detection on {SYMBOL} (REFINED HEAD VALIDATION) ===\n")
print(f"V10: Head must be SIGNIFICANTLY higher than BOTH shoulders (not just barely)\n")
...
print("[V10 DEBUG] Testing L-H-R combinations for Bearish H&S:")
...
for i, (ls_idx, ls_high, ls_prom) in enumerate(prominent_highs[:-2]):
    for j, (h_idx, h_high, h_prom) in enumerate(prominent_highs[i+1:-1], start=i+1):
        for k, (rs_idx, rs_high, rs_prom) in enumerate(prominent_highs[j+1:], start=j+1):
            ...
            # V10: DEBUG - Print this candidate
            ...
            print(f"    Score={score:.2f}, shoulder_diff={shoulder_diff:.1f}%, head_height={head_height:.1f}%")
...
ax1.set_title(f'{SYMBOL} - V10 H&S Detection (REFINED HEAD VALIDATION)\nHead Must Be HIGHEST/LOWEST + Significantly Higher/Lower Than Shoulders',
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v10_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V10] Chart saved to: {output_path}")
