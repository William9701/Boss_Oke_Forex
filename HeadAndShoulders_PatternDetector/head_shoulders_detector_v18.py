...
# Parameters - V18 SWING-BASED NECKLINE
...
print(f"=== V18 Head & Shoulders Detection on {SYMBOL} (SWING-BASED NECKLINE) ===\n")
print(f"Including: ZigZag + Proper Head & Shoulder Detection\n")
print(f"V18: Neckline uses actual swing points (reaction lows/highs between shoulders & head)\n")
...
print(f"[V18] Total swing points for ZigZag: {len(all_swing_highs)} highs + {len(all_swing_lows)} lows\n")
...
print("=" * 80)
print("STEP 5A: INVERSE H&S PATTERN DETECTION (V18)")
print("=" * 80)
...
for i, (ls_idx, ls_low, ls_prom) in enumerate(prominent_lows[:-2]):
    for j, (h_idx, h_low, h_prom) in enumerate(prominent_lows[i+1:-1], start=i+1):
        ...
        for k in range(j+1, max_k):
            ...
            # V18: NECKLINE - The high AFTER each shoulder low
            # For inverse H&S: neckline = high after LS + high after RS
            # Peak1: first swing high AFTER the left shoulder
            peak1_candidates = [(idx, highs_denoised[idx], prom) for idx, high, prom in all_swing_highs
                                if idx > ls_idx]
            if not peak1_candidates:
                continue  # No swing high after LS

            # Pick the FIRST (nearest) swing high after LS
            peak1_idx, peak1_high, _ = min(peak1_candidates, key=lambda x: x[0])

            # Peak2: first swing high AFTER the right shoulder
            peak2_candidates = [(idx, highs_denoised[idx], prom) for idx, high, prom in all_swing_highs
                                if idx > rs_idx]
            if not peak2_candidates:
                continue  # No swing high after RS

            # Pick the FIRST (nearest) swing high after RS
            peak2_idx, peak2_high, _ = min(peak2_candidates, key=lambda x: x[0])
...
if best_inverse_pattern:
    # V18: SNAP shoulders to adjacent swings in ALL_SWING_LOWS (not just prominent)
    print(f"[OK] Found Inverse H&S pattern (Score: {best_score:.2f})")

    # Find the head index in the all_swing_lows list
    head_idx_in_list = None
    for idx, (swing_idx, swing_low, swing_prom) in enumerate(all_swing_lows):
        if swing_idx == best_inverse_pattern['h_idx']:
            head_idx_in_list = idx
            break

    if head_idx_in_list is not None and head_idx_in_list > 0 and head_idx_in_list < len(all_swing_lows) - 1:
        # V18: Snap left shoulder to PREVIOUS swing in ALL swings
        new_ls_idx, new_ls_low, new_ls_prom = all_swing_lows[head_idx_in_list - 1]

        # V18: Snap right shoulder to NEXT swing in ALL swings
        new_rs_idx, new_rs_low, new_rs_prom = all_swing_lows[head_idx_in_list + 1]

        print(f"[V18] SNAPPING shoulders to adjacent swings (using ALL swings):")
        ...
        if h_low < ls_low and h_low < rs_low:
            ...
            # Re-adjust with new head (using all_swing_lows)
            if new_head_idx_in_list > 0 and new_head_idx_in_list < len(all_swing_lows) - 1:
                # New head
                new_h_idx, new_h_low, new_h_prom = all_swing_lows[new_head_idx_in_list]

                # New shoulders (adjacent to new head in ALL swings)
                new_ls_idx, new_ls_low, new_ls_prom = all_swing_lows[new_head_idx_in_list - 1]
                new_rs_idx, new_rs_low, new_rs_prom = all_swing_lows[new_head_idx_in_list + 1]
        ...
        # Peak2: first swing high AFTER right shoulder
        peak2_candidates = [(idx, highs_denoised[idx], prom) for idx, high, prom in all_swing_highs
                            if idx > final_rs_idx]
        if peak2_candidates:
            peak2_idx, peak2_high, _ = min(peak2_candidates, key=lambda x: x[0])
        ...
        print(f"[V18] NECKLINE (swing-based):")
        print(f"      Peak1: idx={peak1_idx}, price={peak1_high:.5f} (first swing high AFTER LS)")
        print(f"      Peak2: idx={peak2_idx}, price={peak2_high:.5f} (first swing high AFTER RS)")
...
print("=" * 80)
print("STEP 5B: BEARISH H&S PATTERN DETECTION (V18)")
print("=" * 80)
...
for i, (ls_idx, ls_high, ls_prom) in enumerate(prominent_highs[:-2]):
    for j, (h_idx, h_high, h_prom) in enumerate(prominent_highs[i+1:-1], start=i+1):
        ...
        for k in range(j+1, max_k):
            ...
            # V18: NECKLINE - The low AFTER each shoulder high
            # For bearish H&S: neckline = low after LS + low after RS
            # Valley1: first swing low AFTER the left shoulder
            valley1_candidates = [(idx, lows_denoised[idx], prom) for idx, low, prom in all_swing_lows
                                  if idx > ls_idx]
            if not valley1_candidates:
                continue  # No swing low after LS

            # Pick the FIRST (nearest) swing low after LS
            valley1_idx, valley1_low, _ = min(valley1_candidates, key=lambda x: x[0])

            # Valley2: first swing low AFTER the right shoulder
            valley2_candidates = [(idx, lows_denoised[idx], prom) for idx, low, prom in all_swing_lows
                                  if idx > rs_idx]
            if not valley2_candidates:
                continue  # No swing low after RS

            # Pick the FIRST (nearest) swing low after RS
            valley2_idx, valley2_low, _ = min(valley2_candidates, key=lambda x: x[0])
...
if best_bearish_pattern:
    # V18: SNAP shoulders to adjacent swings in ALL_SWING_HIGHS (not just prominent)
    print(f"[OK] Found Bearish H&S pattern (Score: {best_score:.2f})")

    # Find the head index in the all_swing_highs list
    head_idx_in_list = None
    for idx, (swing_idx, swing_high, swing_prom) in enumerate(all_swing_highs):
        if swing_idx == best_bearish_pattern['h_idx']:
            head_idx_in_list = idx
            break

    if head_idx_in_list is not None and head_idx_in_list > 0 and head_idx_in_list < len(all_swing_highs) - 1:
        # V18: Snap left shoulder to PREVIOUS swing in ALL swings
        new_ls_idx, new_ls_high, new_ls_prom = all_swing_highs[head_idx_in_list - 1]

        # V18: Snap right shoulder to NEXT swing in ALL swings
        new_rs_idx, new_rs_high, new_rs_prom = all_swing_highs[head_idx_in_list + 1]
        ...
        # V18: POST-SNAP VALIDATION - Head must be HIGHEST among L-H-R
        ...
        if h_high > ls_high and h_high > rs_high:
            ...
            # Re-adjust with new head (using all_swing_highs)
            if new_head_idx_in_list > 0 and new_head_idx_in_list < len(all_swing_highs) - 1:
                # New head
                new_h_idx, new_h_high, new_h_prom = all_swing_highs[new_head_idx_in_list]

                # New shoulders (adjacent to new head in ALL swings)
                new_ls_idx, new_ls_high, new_ls_prom = all_swing_highs[new_head_idx_in_list - 1]
                new_rs_idx, new_rs_high, new_rs_prom = all_swing_highs[new_head_idx_in_list + 1]
        ...
        # V18: Recalculate neckline using SWING LOWS between final shoulders/head
        final_ls_idx = best_bearish_pattern['ls_idx']
        final_h_idx = best_bearish_pattern['h_idx']
        final_rs_idx = best_bearish_pattern['rs_idx']

        # Valley1: first swing low AFTER left shoulder
        valley1_candidates = [(idx, lows_denoised[idx], prom) for idx, low, prom in all_swing_lows
                              if idx > final_ls_idx]
        if valley1_candidates:
            valley1_idx, valley1_low, _ = min(valley1_candidates, key=lambda x: x[0])
        else:
            # Fallback: use absolute min in range
        ...
        # Valley2: first swing low AFTER right shoulder
        valley2_candidates = [(idx, lows_denoised[idx], prom) for idx, low, prom in all_swing_lows
                              if idx > final_rs_idx]
        if valley2_candidates:
            valley2_idx, valley2_low, _ = min(valley2_candidates, key=lambda x: x[0])
        else:
            # Fallback: use absolute min in range
        ...
        print(f"[V18] NECKLINE (swing-based):")
        print(f"      Valley1: idx={valley1_idx}, price={valley1_low:.5f} (first swing low AFTER LS)")
        print(f"      Valley2: idx={valley2_idx}, price={valley2_low:.5f} (first swing low AFTER RS)")
...
for pattern in inverse_hs_patterns:
    ...
    # V18: Neckline from swing highs between shoulders/head, extended forward
    ...
    ax1.plot([times[pattern['peak1_idx']], times[pattern['peak2_idx']], extension_time],
           ...
           'yellow', linewidth=2.5, alpha=0.9, zorder=7, linestyle='--', label='Neckline (Inverse)')

    # V18: Mark the neckline points with diamonds
    ax1.scatter([times[pattern['peak1_idx']], times[pattern['peak2_idx']]],
               [pattern['peak1_price'], pattern['peak2_price']],
               color='yellow', s=200, marker='D', zorder=9, edgecolors='black', linewidths=2,
    ...
    ax1.text(times[h_idx], lows_denoised[h_idx] * 0.98, "INVERSE H&S (V18)", fontsize=11, ha='center',
...
for pattern in bearish_hs_patterns:
    ...
    # V18: Neckline from swing lows between shoulders/head, extended forward
    extension_idx = min(rs_idx + NECKLINE_EXTENSION_BARS, len(times) - 1)
    extension_time = times[extension_idx]
    ...
    ax1.plot([times[pattern['valley1_idx']], times[pattern['valley2_idx']], extension_time],
           ...
           'orange', linewidth=2.5, alpha=0.9, zorder=7, linestyle='--', label='Neckline (Bearish)')

    # V18: Mark the neckline points with diamonds
    ax1.scatter([times[pattern['valley1_idx']], times[pattern['valley2_idx']]],
               [pattern['valley1_price'], pattern['valley2_price']],
               color='orange', s=200, marker='D', zorder=9, edgecolors='black', linewidths=2,
               label='Neckline Points')
    ...
    ax1.text(times[h_idx], highs_denoised[h_idx] * 1.02, "BEARISH H&S (V18)", fontsize=11, ha='center',
...
ax1.set_title(f'{SYMBOL} - V18 H&S Detection (SWING-BASED NECKLINE)\nNeckline = low/high AFTER each shoulder (reaction point)',
...
output_path = rf'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\v18_pairs\{SYMBOL}_v18_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V18] Chart saved to: {output_path}")
