...
# V6: Professional criteria - Lower prominence threshold to catch more RS candidates
...
print(f"=== V6 Head & Shoulders Detection on {SYMBOL} (PRIORITIZE NECKLINE QUALITY) ===\n")
print(f"Including: Wavelet Denoising + Trend + Neckline Slope + RSI + Volume + Extended Neckline\n")
print(f"V6: Modified scoring to prioritize horizontal necklines over perfect shoulder symmetry\n")
...
# V6 DEBUG: Print ALL swing highs before filtering by prominence
all_swing_highs_debug = []
for idx in swing_high_indices:
    prominence = calculate_prominence(highs_denoised, idx, is_high=True)
    all_swing_highs_debug.append((idx, highs_denoised[idx], prominence))

print("\n[V6 DEBUG] ALL swing highs (before prominence filter @ 8%):")
for i, (idx, high, prom) in enumerate(all_swing_highs_debug):
    status = "[OK]" if prom >= MIN_PROMINENCE_SHOULDERS else "[FILTERED]"
    print(f"  {pd.Timestamp(times[idx]).strftime('%Y-%m')} @ {high:.4f} (prom: {prom:.1f}%) {status}")

print("\n[V6 DEBUG] Prominent highs AFTER filter (>=8% prominence):")
for i, (idx, high, prom) in enumerate(prominent_highs):
    print(f"  {i}: {pd.Timestamp(times[idx]).strftime('%Y-%m')} @ {high:.4f} (prom: {prom:.1f}%)")
print()
...
for i in range(len(prominent_highs)):
    ...
    for j in range(i + 1, len(prominent_highs)):
        ...
        # ============= V6 SCORING: PRIORITIZE NECKLINE QUALITY =============
        avg_shoulder = (ls_high + rs_high) / 2.0
        head_height = (h_high - avg_shoulder) / avg_shoulder * 100.0

        # V6: Modified scoring to prioritize horizontal neckline over shoulder symmetry
        score = (head_height * 5.0) + (h_prom * 3.0)
        score -= shoulder_diff * 0.3  # V6: Reduced penalty for shoulder asymmetry (was 1.0)
        score -= neckline_diff * 5.0  # V6: Increased penalty for non-horizontal neckline (was 1.0)
        score += rsi_bonus  # Bonus for RSI divergence
        score += volume_bonus  # Bonus for volume exhaustion
        score -= abs(neckline_angle) * 3.0  # V6: Increased slope penalty (was 0.5)

        # V6 DEBUG: Print scoring for all RS candidates after 2011-08 head
        ls_date = pd.Timestamp(times[ls_idx]).strftime('%Y-%m')
        h_date = pd.Timestamp(times[h_idx]).strftime('%Y-%m')
        rs_date = pd.Timestamp(times[rs_idx]).strftime('%Y-%m')
        if ls_date == "2009-10" and h_date == "2011-08":
            print(f"  [V6 DEBUG] LS={ls_date}, H={h_date}, RS={rs_date}: score={score:.2f}, shoulder_diff={shoulder_diff:.1f}%, neckline_diff={neckline_diff:.1f}%, angle={neckline_angle:.2f}°")
...
fig.suptitle(f'{SYMBOL} - V6 Head & Shoulders Detection (PRIORITIZE NECKLINE QUALITY)\nWith Wavelet Denoising (84.5% accuracy) + Trend + Neckline Slope + RSI + Volume',
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v6_detection.png'
