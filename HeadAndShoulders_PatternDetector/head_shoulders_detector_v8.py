...
print(f"=== V8 Head & Shoulders Detection on {SYMBOL} (HEAD VALIDATION FIX) ===\n")
print(f"Including: ZigZag + Fixed Head Detection (Must be HIGHEST/LOWEST)\n")
print(f"V8: Head must be the absolute HIGHEST high (bearish) or LOWEST low (inverse)\n")
...
print(f"[V8] Total swing points for ZigZag: {len(all_swing_highs)} highs + {len(all_swing_lows)} lows\n")
...
print("=" * 80)
print("STEP 5A: INVERSE H&S PATTERN DETECTION (V8: HEAD MUST BE LOWEST)")
print("=" * 80)
...
for i, (ls_idx, ls_low, ls_prom) in enumerate(prominent_lows[:-2]):
    for j, (h_idx, h_low, h_prom) in enumerate(prominent_lows[i+1:-1], start=i+1):
        for k, (rs_idx, rs_low, rs_prom) in enumerate(prominent_lows[j+1:], start=j+1):
            ...
            # V8: CRITICAL FIX - Head must be the LOWEST point in the entire L-H-R range
            # Check all prominent lows between LS and RS
            is_lowest = True
...
if best_inverse_pattern:
    ...
    print(f"     Head is confirmed as LOWEST point in pattern\n")
else:
    print("[X] No Inverse H&S pattern found\n")

# ============= DETECT BEARISH HEAD AND SHOULDERS =============
print("=" * 80)
print("STEP 5B: BEARISH H&S PATTERN DETECTION (V8: HEAD MUST BE HIGHEST)")
print("=" * 80)
...
for i, (ls_idx, ls_high, ls_prom) in enumerate(prominent_highs[:-2]):
    for j, (h_idx, h_high, h_prom) in enumerate(prominent_highs[i+1:-1], start=i+1):
        for k, (rs_idx, rs_high, rs_prom) in enumerate(prominent_highs[j+1:], start=j+1):
            ...
            # V8: CRITICAL FIX - Head must be the HIGHEST point in the entire L-H-R range
            # Check all prominent highs between LS and RS
            is_highest = True
...
if best_bearish_pattern:
    ...
    print(f"     Head is confirmed as HIGHEST point in pattern\n")
else:
    print("[X] No Bearish H&S pattern found\n")

# ============= V8: VISUALIZATION WITH ZIGZAG =============
print("=" * 80)
...
for pattern in inverse_hs_patterns:
    ...
    ax1.text(times[h_idx], lows_denoised[h_idx] * 0.98, "INVERSE H&S (V8)", fontsize=11, ha='center',
...
for pattern in bearish_hs_patterns:
    ...
    ax1.text(times[h_idx], highs_denoised[h_idx] * 1.02, "BEARISH H&S (V8)", fontsize=11, ha='center',
...
ax1.set_title(f'{SYMBOL} - V8 H&S Detection (HEAD VALIDATION FIX)\nZigZag + Head Must Be HIGHEST/LOWEST Point',
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v8_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V8] Chart saved to: {output_path}")
