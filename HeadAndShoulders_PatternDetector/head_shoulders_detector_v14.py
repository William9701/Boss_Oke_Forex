...
# Parameters - V14 WITH ADJACENT SWING SHOULDERS
SYMBOL = "EURUSD"
...
print(f"=== V14 Head & Shoulders Detection on {SYMBOL} (ADJACENT SWING SHOULDERS) ===\n")
print(f"Including: ZigZag + Proper Head & Shoulder Detection\n")
print(f"V14: Shoulders are IMMEDIATELY adjacent swings (prev & next) to head\n")
...
print("=" * 80)
print("STEP 5A: INVERSE H&S PATTERN DETECTION (V14)")
print("=" * 80)

best_inverse_pattern = None
best_score = 0

# V14: Shoulders are IMMEDIATELY adjacent swings to head
# For each potential head, check if prev and next swings can be shoulders
for j in range(1, len(prominent_lows) - 1):  # Head must have swings before and after
    h_idx, h_low, h_prom = prominent_lows[j]

    # V14: Left shoulder = PREVIOUS swing (j-1)
    ls_idx, ls_low, ls_prom = prominent_lows[j - 1]

    # V14: Right shoulder = NEXT swing (j+1)
    rs_idx, rs_low, rs_prom = prominent_lows[j + 1]
    ...
    # V14: Head must be LOWER than BOTH shoulders
...
print("=" * 80)
print("STEP 5B: BEARISH H&S PATTERN DETECTION (V14)")
print("=" * 80)
...
# V14: Shoulders are IMMEDIATELY adjacent swings to head
for j in range(1, len(prominent_highs) - 1):  # Head must have swings before and after
    h_idx, h_high, h_prom = prominent_highs[j]

    # V14: Left shoulder = PREVIOUS swing (j-1)
    ls_idx, ls_high, ls_prom = prominent_highs[j - 1]

    # V14: Right shoulder = NEXT swing (j+1)
    rs_idx, rs_high, rs_prom = prominent_highs[j + 1]
    ...
    # V14: Head must be HIGHER than BOTH shoulders
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v14_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V14] Chart saved to: {output_path}")
