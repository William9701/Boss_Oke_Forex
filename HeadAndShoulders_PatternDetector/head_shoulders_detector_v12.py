...
# Parameters - V12 WITH BALANCED HEAD + SHOULDER SELECTION
SYMBOL = "AUDUSD"
LOOKBACK_BARS = 240  # 20 years
SWING_ORDER = 5

# V12: Professional criteria
...
print(f"=== V12 Head & Shoulders Detection on {SYMBOL} (BALANCED SELECTION) ===\n")
print(f"V12: Find pattern with HIGHEST head + closest RS to head for better structure\n")
...
# ============= WAVELET DENOISING =============
print("=" * 80)
print("STEP 0: WAVELET DENOISING")
print("=" * 80)
...
print(f"[OK] RSI calculated\n")

# ============= CALCULATE VOLUME =============
print("=" * 80)
print("STEP 3: VOLUME CALCULATION")
print("=" * 80)
...
print(f"[OK] Volume SMA calculated\n")
...
print(f"[OK] Filtered: {len(prominent_highs)} highs, {len(prominent_lows)} lows\n")
...
# ============= INVERSE H&S =============
print("=" * 80)
print("STEP 5A: INVERSE H&S DETECTION")
print("=" * 80)
...
for i, (ls_idx, ls_low, ls_prom) in enumerate(prominent_lows[:-2]):
    for j, (h_idx, h_low, h_prom) in enumerate(prominent_lows[i+1:-1], start=i+1):
        for k, (rs_idx, rs_low, rs_prom) in enumerate(prominent_lows[j+1:], start=j+1):
            ...
            # V10: Head must be SIGNIFICANTLY lower than BOTH shoulders
            ...
            # V10: Strict check - head must be THE LOWEST of the three
...
if best_inverse_pattern:
    ...
    print(f"[OK] Found Inverse H&S (Score: {best_score:.2f})\n")
else:
    print("[X] No Inverse H&S found\n")

# ============= BEARISH H&S =============
print("=" * 80)
print("STEP 5B: BEARISH H&S DETECTION (V10)")
print("=" * 80)

best_bearish_pattern = None
best_score = 0

print("[V12 DEBUG] Testing L-H-R combinations for Bearish H&S:")
...
for i, (ls_idx, ls_high, ls_prom) in enumerate(prominent_highs[:-2]):
    for j, (h_idx, h_high, h_prom) in enumerate(prominent_highs[i+1:-1], start=i+1):
        for k, (rs_idx, rs_high, rs_prom) in enumerate(prominent_highs[j+1:], start=j+1):
            ...
            # V10: CRITICAL - Head must be SIGNIFICANTLY higher than BOTH shoulders
            # This ensures head is the peak, not a lower high
            ...
            # V10: DOUBLE CHECK - Head must be THE HIGHEST of the three points
            # This prevents selecting a lower high as the head when a higher peak exists
            ...
            # V12: BALANCED SCORING - Highest head wins, but penalize overly compressed patterns
            bars_h_to_rs = rs_idx - h_idx
            bars_ls_to_h = h_idx - ls_idx

            # V12: Penalty for patterns that are TOO compressed (RS too close to head)
            # Prefer RS at reasonable distance (around 50-80 bars on monthly)
            compression_penalty = 0
            if bars_h_to_rs < 30:  # RS too close to head (less than 2.5 years on monthly)
                compression_penalty = (30 - bars_h_to_rs) * 5.0  # Heavy penalty

            score = (head_height * 5.0) + (h_prom * 10.0)
            score += (h_high * 100.0)  # Head price bonus (dominates)
            score -= compression_penalty  # V12: Penalize compressed patterns
            score -= shoulder_diff * 0.2  # V12: Reduced to minimize symmetry impact
            score -= neckline_diff * 5.0
            if has_rsi_div:
                score += 50.0
            if has_vol_exhaust:
            ...
            # V12: DEBUG - Print this candidate
            ls_time = pd.Timestamp(times[ls_idx]).strftime('%Y-%m')
            h_time = pd.Timestamp(times[h_idx]).strftime('%Y-%m')
            rs_time = pd.Timestamp(times[rs_idx]).strftime('%Y-%m')

            print(f"  Candidate: LS={ls_time}@{ls_high:.4f}, H={h_time}@{h_high:.4f}, RS={rs_time}@{rs_high:.4f}")
            print(f"    Score={score:.2f}, bars_H_to_RS={bars_h_to_rs}, compress_penalty={compression_penalty:.1f}, shoulder_diff={shoulder_diff:.1f}%")
...
if best_bearish_pattern:
    ...
    print(f"\n[OK] Found Bearish H&S (Score: {best_score:.2f})")
    ls_time = pd.Timestamp(times[best_bearish_pattern['ls_idx']]).strftime('%Y-%m')
    h_time = pd.Timestamp(times[best_bearish_pattern['h_idx']]).strftime('%Y-%m')
    rs_time = pd.Timestamp(times[best_bearish_pattern['rs_idx']]).strftime('%Y-%m')
    print(f"  LS={ls_time}, H={h_time}, RS={rs_time}\n")
else:
    print("\n[X] No Bearish H&S found\n")

# ============= VISUALIZATION =============
print("=" * 80)
print("STEP 6: CHART VISUALIZATION")
print("=" * 80)
...
if len(all_swings) > 0:
    ...
    ax1.plot(zigzag_times, zigzag_prices, 'cyan', linewidth=2, alpha=0.8,
             label='ZigZag', zorder=4, linestyle='-', marker='o', markersize=5)
...
ax1.plot(times[valid_sma], sma_50[valid_sma], 'purple', linewidth=2, alpha=0.6, label='50 SMA')
...
for pattern in inverse_hs_patterns:
    ...
    ax1.plot([times[pattern['peak1_idx']], times[pattern['peak2_idx']], extension_time],
           ...
           'yellow', linewidth=2, alpha=0.9, zorder=7, linestyle='--')
    ...
    ax1.text(times[h_idx], lows_denoised[h_idx] * 0.98, "INVERSE H&S (V10)", fontsize=11, ha='center',
...
for pattern in bearish_hs_patterns:
    ...
    ax1.plot([times[pattern['valley1_idx']], times[pattern['valley2_idx']], extension_time],
           ...
           'orange', linewidth=2, alpha=0.9, zorder=7, linestyle='--')
    ...
    ax1.text(times[h_idx], highs_denoised[h_idx] * 1.02, "BEARISH H&S (V10)", fontsize=11, ha='center',
...
ax1.set_title(f'{SYMBOL} - V12 H&S Detection (BALANCED SELECTION)\nHighest Head + Closest Right Shoulder for Better Structure',
...
valid_vol = ~np.isnan(volume_sma_20)
ax3.plot(times[valid_vol], volume_sma_20[valid_vol], 'orange', linewidth=1.5, label='Vol SMA(20)')
...
ax4.fill_between(times, 0, price_vs_sma, where=(price_vs_sma < 0), color='red', alpha=0.3, label='Downtrend')
ax4.fill_between(times, 0, price_vs_sma, where=(price_vs_sma > 0), color='green', alpha=0.3, label='Uptrend')
...
output_path = r'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\head_shoulders_v12_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V12] Chart saved to: {output_path}")
