...
print(f"=== V17 Head & Shoulders Detection on {SYMBOL} (SNAP TO ALL SWINGS) ===\n")
print(f"Including: ZigZag + Proper Head & Shoulder Detection\n")
print(f"V17: Detect > Snap to NEXT swing in time (not just prominent) > Validate > Re-adjust\n")
...
if best_inverse_pattern:
    # V17: SNAP shoulders to adjacent swings in ALL_SWING_LOWS (not just prominent)
    ...
    if head_idx_in_list is not None and head_idx_in_list > 0 and head_idx_in_list < len(all_swing_lows) - 1:
        # V17: Snap left shoulder to PREVIOUS swing in ALL swings
        new_ls_idx, new_ls_low, new_ls_prom = all_swing_lows[head_idx_in_list - 1]

        # V17: Snap right shoulder to NEXT swing in ALL swings
        new_rs_idx, new_rs_low, new_rs_prom = all_swing_lows[head_idx_in_list + 1]

        print(f"[V17] SNAPPING shoulders to adjacent swings (using ALL swings):")
...
if best_bearish_pattern:
    # V17: SNAP shoulders to adjacent swings in ALL_SWING_HIGHS (not just prominent)
    ...
    if head_idx_in_list is not None and head_idx_in_list > 0 and head_idx_in_list < len(all_swing_highs) - 1:
        ...
        # V17: POST-SNAP VALIDATION - Head must be HIGHEST among L-H-R
        ...
        print(f"[V17] NECKLINE after re-adjustment:")
...
output_path = rf'C:\Users\ASUS\Desktop\Boss_Oke_Forex\HeadAndShoulders_PatternDetector\v17_pairs\{SYMBOL}_v17_detection.png'
plt.savefig(output_path, dpi=150, bbox_inches='tight')
print(f"\n[V17] Chart saved to: {output_path}")
